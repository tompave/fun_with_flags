...
* Improve error handling in different layers of the package. From the persistence adapter all the way to the public functions of the top-level package. In practice, this means that some situations that would have caused a `MatchError` now bubble up an error tuple. Most importantly, this does **not** affect the signature or behaviour of the `FunWithFlags.enabled?/2` function, which continues to return a simple boolean. ([pull/120](https://github.com/tompave/fun_with_flags/pull/120))
