defmodule FunWithFlags.TelemetryTest do
  ...
  describe ":enable telemetry" do
    ...
    test "emits telemetry event when enabling a flag for an actor", %{
      flag_name: flag_name,
      scrooge: scrooge
    } do
    ...
    test "emits telemetry event when enabling a flag for percentage of time", %{
      flag_name: flag_name
    } do
      ratio = 0.5
    ...
    test "emits telemetry event when enabling a flag for percentage of actors", %{
      flag_name: flag_name
    } do
      ratio = 0.5
  ...
  describe "enabled? telemetry" do
    ...
    test "emits telemetry event when checking an enabled flag for a specific actor", %{
      flag_name: flag_name,
      scrooge: scrooge
    } do
      FunWithFlags.enable(flag_name, for_actor: scrooge)
    ...
    test "emits telemetry event when checking a disabled flag for a specific actor", %{
      flag_name: flag_name,
      donald: donald
    } do
      FunWithFlags.enable(flag_name)
    ...
    test "emits telemetry event when checking an enabled flag for a group", %{
      flag_name: flag_name,
      scrooge: scrooge
    } do
      group = :billionaires
  ...
  describe "disable telemetry" do
    ...
    test "emits telemetry event when disabling a flag for an actor", %{
      flag_name: flag_name,
      scrooge: scrooge
    } do
      FunWithFlags.enable(flag_name, for_actor: scrooge)
    ...
    test "emits telemetry event when disabling a flag for a percentage of time", %{
      flag_name: flag_name
    } do
      ratio = 0.5
      FunWithFlags.enable(flag_name, for_percentage_of: {:time, ratio})
    ...
    test "emits telemetry event when disabling a flag for a percentage of actors", %{
      flag_name: flag_name
    } do
      ratio = 0.5
      FunWithFlags.enable(flag_name, for_percentage_of: {:actors, ratio})
  ...
  describe "clear telemetry" do
    ...
    test "emits telemetry event when clearing a flag for an actor", %{
      flag_name: flag_name,
      scrooge: scrooge
    } do
      FunWithFlags.enable(flag_name, for_actor: scrooge)
    ...
    test "emits telemetry event when clearing fails for an actor", %{
      flag_name: flag_name,
      scrooge: scrooge
    } do
      error_reason = {:error, :test_error}
    ...
    defp attach_telemetry do
      :telemetry.attach_many(
        ...
        fn event_name, measurements, metadata, reply_to ->
          send(reply_to, %{event: event_name, measurements: measurements, metadata: metadata})
        end,
        self()
      )
